...
open class FRadioPlayer: NSObject {
    ...
    private override init() {
        ...
        try? audioSession.setCategory(AVAudioSession.Category.playback, mode: AVAudioSession.Mode.default, options: [.defaultToSpeaker, .allowBluetooth, .allowAirPlay, .mixWithOthers, .interruptSpokenAudioAndMixWithOthers])
    ...
    open func changeCurrentTime(time: CMTime) {
                        
        self.player?.seek(to: time)
        
    }
    
    open func durationInSeconds() -> Float {//self.playerItem
        if let _playerItem = player?.currentItem, _playerItem.duration != CMTime.indefinite {
     ...
     */
    open func currentPlayItem() -> AVPlayerItem?  {
